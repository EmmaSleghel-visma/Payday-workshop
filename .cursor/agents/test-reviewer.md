...
readonly: true
---
<!-- GENERATED from agents/test-reviewer.md by agents/_generate.py — do not edit directly. -->
...
- **`test-generator`** — Apply the review findings. Ask it: *Apply the review findings above. Re-execute any scenario whose assertion you change, and do not weaken an assertion to make a test pass.*
- **`test-healer`** — Diagnose a real failure. Ask it: *The review suggests the app may be at fault rather than the test. Run the suite, debug the failure in the browser, and decide whether the TEST is stale or the APP is broken before changing anything.*
